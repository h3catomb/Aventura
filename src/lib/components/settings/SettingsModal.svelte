...
>
  <div
    class="card w-full max-w-5xl h-[85vh] overflow-hidden flex flex-col shadow-2xl"
  ...
  >
    ...
    >
      <div class="flex items-center gap-2">
        <button
          class="md:hidden btn-ghost rounded-lg p-1 -ml-4"
        ...
        <button
          class="hidden md:flex btn-ghost rounded-lg p-1 -ml-1 text-surface-400 hover:text-surface-200"
        ...
        <h2
          class="text-lg font-semibold text-surface-100 flex items-center gap-2 -ml-4 md:ml-0"
        >
    ...
    <div class="flex flex-1 min-h-0 relative">
      ...
      <div
        class="absolute z-30 h-full w-64 border-r border-surface-700 overflow-y-auto bg-surface-800 flex flex-col p-3 gap-1 flex-shrink-0 transition-transform duration-200 md:hidden"
        class:-translate-x-full={!isMobileMenuOpen}
        class:translate-x-0={isMobileMenuOpen}
      >
        {#each tabs as tab}
          <button
            class="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium transition-all text-left"
            class:bg-surface-700={activeTab === tab.id}
            class:text-surface-100={activeTab === tab.id}
            class:text-surface-400={activeTab !== tab.id}
            class:hover:bg-surface-700={activeTab !== tab.id}
          ...
          >
            <tab.icon class="h-4 w-4 opacity-70" />
            <span>{tab.label}</span>
          </button>
        {/each}
        ...
        <button
          class="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all text-left mt-auto"
          onclick={() => {
            handleResetAll();
            isMobileMenuOpen = false;
          }}
        >
          <RotateCcw class="h-4 w-4 opacity-70" />
          <span>Reset All</span>
        </button>
      </div>
      ...
      <div
        class="hidden md:flex flex-col border-r border-surface-700 bg-surface-800/30 overflow-y-auto p-3 gap-1 flex-shrink-0 transition-all duration-300 ease-in-out"
        class:w-64={!isSidebarCollapsed}
        class:w-16={isSidebarCollapsed}
      >
        {#each tabs as tab}
          <button
            class="flex items-center rounded-lg py-3 text-sm font-medium transition-all"
          ...
          >
            <tab.icon class="h-4 w-4 opacity-70 flex-shrink-0" />
            <span
              class="ml-3 transition-opacity duration-200 whitespace-nowrap overflow-hidden"
        ...
        <button
          class="flex items-center rounded-lg py-3 text-sm font-medium text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all mt-auto"
          class:justify-start={!isSidebarCollapsed}
          class:justify-center={isSidebarCollapsed}
          class:px-4={!isSidebarCollapsed}
          class:px-0={isSidebarCollapsed}
          onclick={handleResetAll}
          title={isSidebarCollapsed ? "Reset All" : undefined}
        >
          <RotateCcw class="h-4 w-4 opacity-70 flex-shrink-0" />
          <span
            class="ml-3 transition-opacity duration-200 whitespace-nowrap overflow-hidden"
            class:opacity-100={!isSidebarCollapsed}
            class:opacity-0={isSidebarCollapsed}
      ...
      <!-- Content Area -->
      <div class="flex-1 overflow-y-auto p-3 sm:p-8 bg-surface-900/50 min-w-0">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <p class="mb-3 text-xs text-surface-500">
                  Manage your API endpoint configurations. Each profile can have
                  its own URL, API key, and model list. Select which profile to
                  use for each service in the Generation and Advanced tabs.
                </p>
                ...
                <div class="card bg-surface-900 p-3 mb-3">
                  ...
                  {#if updateInfo?.available}
                    <div class="mt-3 pt-3 border-t border-surface-700">
                      {#if updateInfo.body}
                        <p class="text-xs text-surface-400 mb-3 line-clamp-3">
                          {updateInfo.body}
                        </p>
                      {/if}
              ...
              {#if settings.systemServicesSettings.imageGeneration.enabled}
                ...
                <div class="space-y-2">
                  ...
                  <select
                    ...
                    onchange={(e) => {
                      settings.systemServicesSettings.imageGeneration.size = e
                        .currentTarget.value as "512x512" | "1024x1024";
                      settings.saveSystemServicesSettings();
                    }}
                  >
                ...
                {#if settings.systemServicesSettings.imageGeneration.portraitMode}
                  ...
                  <div class="space-y-2">
                    ...
                    <p class="text-xs text-surface-500">
                      Model used when generating character portraits from visual
                      descriptors.
                    </p>
                    <input
                      type="text"
                  ...
                  <div class="space-y-2">
                    ...
                    <p class="text-xs text-surface-500">
                      Model used for story images when a character portrait is
