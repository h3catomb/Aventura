...
export class FandomService {
  ...
  async getSection(wiki: string, title: string, sectionIndex: string): Promise<FandomSectionContent> {
    ...
    try {
      ...
      if (typeof parseText === 'string') {
        ...
        throw new Error(`No text content returned for section ${sectionIndex}. The section may be empty or the wiki may restrict API access.`);
      } else {
      ...
      const sectionTitle = sectionTitleMatch ? sectionTitleMatch[1].trim() : (sectionIndex === '0' ? 'Introduction' : `Section ${sectionIndex}`);
  ...
  htmlToPlainText(html: string): string {
    ...
    text = text.replace(/<div[^>]*class="[^"]*(?:navbox|infobox|toc|mw-references|reference|reflist|noprint|catlinks|messagebox)[^"]*"[^>]*>[\s\S]*?<\/div>/gi, '');
