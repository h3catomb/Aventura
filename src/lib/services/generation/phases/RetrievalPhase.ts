...
import type {
  GenerationContext, GenerationEvent, PhaseStartEvent,
  PhaseCompleteEvent, RetrievalResult, AbortedEvent,
} from '../types';
...
export interface RetrievalDependencies {
  ...
  answerChapterQuestion: (
    chapterNumber: number, question: string,
    chapters: GenerationContext['worldState']['chapters']
  ) => Promise<string>;
  answerChapterRangeQuestion: (
    startChapter: number, endChapter: number, question: string,
    chapters: GenerationContext['worldState']['chapters']
  ) => Promise<string>;
...
export class RetrievalPhase {
  async *execute(input: RetrievalInput): AsyncGenerator<GenerationEvent, RetrievalResult> {
    ...
    if (chapters.length > 0 && timelineFillEnabled && memoryConfig.enableRetrieval) {
      tasks.push(this.runMemoryRetrieval(input).then(result => {
    ...
    if (hasLoreContent) {
      tasks.push(
        dependencies.getRelevantLorebookEntries(
          ...
          { characters, locations, items }, activationTracker, abortSignal
        ).then(result => { lorebookContext = result.contextBlock; })
          .catch(err => {
            if (err instanceof Error && err.name === 'AbortError') return;
    ...
    if (abortSignal?.aborted) {
      ...
      return { chapterContext: null, lorebookContext: null, timelineFillResult: null, combinedContext: null };
    }

    const combinedContext = [chapterContext, lorebookContext].filter(Boolean).join('\n') || null;
    const result: RetrievalResult = { chapterContext, lorebookContext, timelineFillResult, combinedContext };
  ...
  private async runMemoryRetrieval(input: RetrievalInput): Promise<{
    ...
    if (dependencies.shouldUseAgenticRetrieval(chapters.length)) {
      const result = await dependencies.runAgenticRetrieval(
        userAction.content, visibleEntries, chapters, lorebookEntries,
        ...
        abortSignal, storyMode, pov, tense
      );
      return {
    ...
    return { chapterContext: null, timelineFillResult: await dependencies.runTimelineFill(visibleEntries, chapters) };
  }
}
